...
module.exports = function (opts) {
  ...
  // must exist
  if (!opts.dir) throw new Error('data directory opts.dir required')
  ...
  function logFunction (data, cb) {
    if (!data) {
      return setImmediate(function () {
        cb(new Error('log function called with falsy data'))
      })
    }
    ...
    if (typeof data === 'object') {
      ...
      // data must have delimiter or i add it.
      var d = delim
  ...
  // expose for tranparency
  logFunction._q = _q
  logFunction.getState = function () {
    return logState
  }
